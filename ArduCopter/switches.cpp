...
#if SPRAYER == ENABLED
        case AUXSW_SPRAYER:
            sprayer.set_aux_mode(ch_flag);
            switch(ch_flag)
            {
            case AUX_SWITCH_LOW:
                GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_INFO, "AUXSW_SPRAYER = OFF");
                break;
            case AUX_SWITCH_MIDDLE:
                GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_INFO, "AUXSW_SPRAYER = MANUAL");
                break;
            case AUX_SWITCH_HIGH:
                GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_INFO, "AUXSW_SPRAYER = AUTO");
                break;
            }

            // if we are disarmed the pilot must want to test the pump
            sprayer.test_pump((ch_flag == AUX_SWITCH_HIGH) && !motors.armed());
            if (sprayer.get_test_pump()) {
            	GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_INFO, "AUXSW_SPRAYER = TEST PUMP");
            }
            break;
#endif
