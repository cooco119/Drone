...
void Copter::rtl_climb_start()
{
    ...
    // set the destination
#if defined(JJJ_ADJUST_ALTITUDE)
    if(!wp_nav.set_wp_destination_adjust_alt(rtl_path.climb_target))
#else
...
#if defined(JJJ_ADJUST_ALTITUDE)
    if(!wp_nav.set_wp_destination_adjust_alt(rtl_path.return_target))
#else
