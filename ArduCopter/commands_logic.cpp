...
#if SPRAYER == ENABLED
    case MAV_CMD_DO_SPRAYER:                           // Mission command to configure or release parachute
        do_sprayer(cmd);
        break;
#endif
...
#if SPRAYER == ENABLED
// do_sprayer - configure or release sprayer
void Copter::do_sprayer(const AP_Mission::Mission_Command& cmd)
{
    switch (cmd.p1) {
        case SPRAYER_DISABLE:
            sprayer.enable(false);
            Log_Write_Event(DATA_SPRAYER_DISABLED);
            break;
        case SPRAYER_ENABLE:
            sprayer.enable(true);
            Log_Write_Event(DATA_SPRAYER_ENABLED);
            break;
        default:
            // do nothing
            break;
...
void Copter::do_gripper(const AP_Mission::Mission_Command& cmd)
{
  ...
  #if defined(JJJ_AUTOMATIC_YAW_BEHAVIOR)
    switch (cmd.content.gripper.num) {
        case GRIPPER_ACTION_RELEASE:
            g.wp_yaw_behavior = WP_YAW_BEHAVIOR_NONE;
            Log_Write_Event(DATA_EPM_RELEASE);
            break;
        case GRIPPER_ACTION_GRAB:
        default:
            //g.wp_yaw_behavior_org = g.wp_yaw_behavior;
            g.wp_yaw_behavior = cmd.content.gripper.num;
